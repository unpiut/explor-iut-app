            ...
            ? (
               <>
                  ...
                  <div className={`ring rounded ${!droite.disable ? 'bg-white text-blue-900 animate-softBlink ring-blue-900' : 'ring-gray-400'}  m-4 items-center flex justify-self-end h-3/5`}>
                     <button type="button" onClick={onClick} className={`${!droite.disable ? 'animate-softBlink' : 'text-gray-400'} rounded sm:text-2xl px-2 text-sm sm:font-bold`}>Etape suivante</button>
                     <img className="lg:block hidden" width={30} style={{ transform: 'rotate(-0.25turn)' }} src={fleche} alt="fleche" />
                  </div>
               </>
            )
         ...
         {gauche && droite && !onClick
            ? (
               <>
                  ...
                  <div className={`ring rounded ${!droite.disable ? 'bg-white text-blue-900 animate-softBlink ring-blue-900' : 'ring-gray-400'}  m-4 items-center flex justify-self-end h-3/5`}>
                     <Link onClick={onClick} className={`${!droite.disable ? 'animate-softBlink' : 'text-gray-400'} rounded sm:text-2xl px-2 text-sm sm:font-bold no-underline`} to={!droite.disable ? droite.lien : droite.lienActu}>Etape suivante</Link>
                     <img className="lg:block hidden" width={30} style={{ transform: 'rotate(-0.25turn)' }} src={fleche} alt="fleche" />
                  </div>
               </>
            )
